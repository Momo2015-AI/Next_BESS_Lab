...
- [ ] 2. 后端：实现 `/api/pipeline/calculate` 路由 (R8.1, R8.3, R4.6)
...
- [ ] 3. 检查点 - 后端 pipeline API 就绪，所有测试通过

- [ ] 4. 前端：安装 Pinia 并创建全局 Store (R7.1, R7.3)
...
- [ ] 5. 前端：搭建 Vue Router 五阶段路由 (R1.1, R7.2)
...
- [ ] 6. 检查点 - 前端路由框架就绪，可通过 URL 导航到各阶段页面

- [ ] 7. 前端：重构首页为流程总览仪表盘 (R1.4)
...
- [ ] 9. 前端：创建 Phase1Page 项目立项页面 (R2.1, R2.2, R2.3, R2.4, R2.5)
...
- [ ] 10. 前端：创建 Phase2Page 系统设计页面 (R3.1, R3.2, R3.3, R3.4)
...
- [ ] 11. 前端：创建 Phase3Page 性能分析页面 (R4.1, R4.2, R4.3, R4.4)
...
- [ ] 12. 前端：创建 Phase4Page 经济评估页面 (R5.1, R5.2, R5.3, R5.4, R5.5)
...
- [ ] 13. 前端：创建 Phase5Page 成果输出页面 (R6.1, R6.2, R6.3, R6.4)
...
- [ ] 15. 前端：完善 Phase 子步骤导航与进度追踪 (R1.2, R1.3)
