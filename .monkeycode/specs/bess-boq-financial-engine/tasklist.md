...
- [ ] 2. 后端：创建完整财务计算引擎 (R2.1, R2.3, R2.4, R2.5, R2.6, R2.7, R2.8, R2.9)
...
- [ ] 3. 后端：实现 `/api/financial/calculate` 路由 (R2.1)
...
- [ ] 4. 检查点 - 财务引擎后端 API 可独立调通

- [ ] 5. 后端：数据库迁移 — 新增 BOQ 表与扩展现有表 (R1, R3.1, R3.3, R4)
...
- [ ] 6. 后端：种子数据 — 初始化 7 级 BOQ 分类模板 (R1.2)
...
- [ ] 7. 后端：实现 BOQ CRUD API (R1.3, R1.5, R5.1, R5.2)
...
- [ ] 8. 后端：重构 pipeline.py 财务计算委托到新引擎 (R2.10)
...
- [ ] 9. 检查点 - 后端所有 API 就绪，`/api/pipeline/calculate` 输出兼容，新旧并存

- [ ] 10. 前端：创建单位工具模块 (R3.4, R3.5, R3.6)
...
- [ ] 11. 前端：扩展 BESS Store — BOQ 与财务状态 (R1, R2, R5)
...
- [ ] 12. 前端：创建 BOQ 编辑器组件 (R1.1, R1.3, R1.4, R1.5, R1.6, R5.1, R5.2, R5.3)
...
- [ ] 13. 前端：重构 FinancialDashboard — 移除前端计算、接入后端 API (R2.1, R6.1, R6.2, R6.3, R6.4)
...
- [ ] 14. 前端：重构 Phase4Page — BOQ 先行 + 财务仪表盘整合 (R5.1, R6.1)
...
- [ ] 15. 前端：扩展调研表 — 中东并网与环境字段 (R4.1, R4.2, R4.3)
