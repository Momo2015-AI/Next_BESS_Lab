...
def get_simulation_results(version_id):
    ...
    results = SimulationResult.query.filter_by(version_id=version_id)\
...
def delete_simulation_result(result_id):
    """删除仿真结果"""
    from database import db, SimulationResult
...
def create_correction_template():
    ...
    template = CorrectionTemplate(
        ...
        annual_corrections=json.dumps(data.get('annual_corrections', {})) if isinstance(data.get('annual_corrections'), dict) else json.dumps({}),
    ...
    if template.is_default:
        CorrectionTemplate.query.filter_by(tenant_id=user.tenant_id)\
...
def update_correction_template(template_id):
    ...
    if 'annual_corrections' in data:
        template.annual_corrections = json.dumps(data['annual_corrections']) if isinstance(data['annual_corrections'], dict) else data['annual_corrections']
