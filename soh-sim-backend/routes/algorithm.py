    ...
    return [
        ...
        # ========== 财务类 (financial) ==========
        {
            ...
            'parameters': {
                ...
                'equity_ratio': {'label': '权益融资比例', 'default': 30, 'min': 10, 'max': 50, 'unit': '%'},
                'loan_rate': {'label': '贷款利率', 'default': 4.5, 'min': 2, 'max': 10, 'unit': '%'},
                'cost_of_equity': {'label': '权益成本', 'default': 12, 'min': 8, 'max': 20, 'unit': '%'},
                'loan_years': {'label': '贷款年限', 'default': 15, 'min': 5, 'max': 25, 'unit': '年'},
                'tax_rate': {'label': '所得税率', 'default': 25, 'min': 0, 'max': 35, 'unit': '%'},
                'vat_rate': {'label': 'VAT税率', 'default': 5, 'min': 0, 'max': 20, 'unit': '%'},
                'depreciation_years': {'label': '折旧年限', 'default': 20, 'min': 10, 'max': 25, 'unit': '年'},
                'residual_rate': {'label': '残值率', 'default': 5, 'min': 0, 'max': 20, 'unit': '%'},
                'depreciation_method': {'label': '折旧方法', 'default': 'straight-line', 'min': '', 'max': '', 'unit': ''},
            },
            'accuracy_level': 'high',
            'accuracy_desc': '收敛精度 < 1e-6',
            'category': 'financial',
            'is_builtin': True,
            'description': '内部收益率（IRR）是使项目净现值为零时的折现率，是投资决策的核心指标。采用Newton-Raphson迭代法数值求解f(r)=0的根，收敛精度 < 1e-6。行业标准：全投资IRR（Project IRR）目标6-10%，自有资金IRR（Equity IRR）目标10-15%。中东Masdar项目典型融资结构：债务融资65-75%+权益35-25%，贷款利率3.5-5%（中东主权基金支持项目利率较低），权益成本10-14%。VAT税率5%为阿联酋标准税率。支持直线折旧和双倍余额递减法两种折旧方式。',
        },
        {
            'name': 'LCOS 平准化储能成本',
            ...
            'parameters': {
                'container_cost': {'label': '集装箱单价', 'default': 100, 'min': 60, 'max': 180, 'unit': '万元/MWh'},
                'pcs_cost': {'label': 'PCS单价', 'default': 25, 'min': 15, 'max': 50, 'unit': '万元/MW'},
                'bop_cost': {'label': 'BOP配套成本', 'default': 30, 'min': 10, 'max': 50, 'unit': '万元/MWh'},
                'substation_cost': {'label': '变电站成本', 'default': 15, 'min': 5, 'max': 30, 'unit': '万元/MW'},
                'transmission_cost': {'label': '输电线路成本', 'default': 10, 'min': 3, 'max': 25, 'unit': '万元/MW'},
                'land_cost': {'label': '土地成本', 'default': 8, 'min': 2, 'max': 20, 'unit': '万元/MW'},
                'dev_cost': {'label': '开发费', 'default': 20, 'min': 5, 'max': 40, 'unit': '万元/MW'},
                'fixed_om': {'label': '固定O&M', 'default': 35, 'min': 15, 'max': 60, 'unit': '元/kW·年'},
                'var_om': {'label': '可变O&M', 'default': 3, 'min': 1, 'max': 8, 'unit': '元/MWh'},
                'insurance_rate': {'label': '保险费率', 'default': 0.4, 'min': 0.1, 'max': 0.8, 'unit': '% of CAPEX'},
                'om_escalation': {'label': 'O&M年涨幅', 'default': 2, 'min': 0, 'max': 5, 'unit': '%/年'},
                'vat_rate': {'label': 'VAT税率', 'default': 5, 'min': 0, 'max': 20, 'unit': '%'},
                'decommissioning_cost': {'label': '退役成本', 'default': 10, 'min': 5, 'max': 20, 'unit': '万元/MWh'},
            },
            'accuracy_level': 'medium',
            'accuracy_desc': '行业对标标准',
            'category': 'financial',
            'is_builtin': True,
            'description': '平准化储能成本（LCOS）是储能项目经济性的核心对标指标，类比光伏LCOE。计算全生命周期内每放出1kWh电的折现总成本，包含初始投资+运维支出+增容支出，扣除末期残值。\n\n【中东Masdar标准】：\n- 集装箱成本：90-110万元/MWh（LFP长时储能）\n- PCS成本：20-30万元/MW\n- 变电站：10-20万元/MW（含132kV/33kV变压设备）\n- 输电线路：5-15万元/MW（接入电网距离相关）\n- 土地：5-12万元/MW（沙漠地区成本较低）\n- LCOS目标：< 0.06美元/kWh（折合约0.40元/kWh）\n\n【SPPC沙特标准】：\n- 固定O&M：30-40元/kW·年\n- 保险费率：0.3-0.5% of CAPEX\n- 退役成本：5-15万元/MWh（含环保处理费用）\n\n2024年LFP电芯价格触底后，全球储能CAPEX成本普遍下降约20-30%。',
        },
        {
            'name': 'DSCR 偿债覆盖倍率',
            ...
            'parameters': {
                ...
                'dscr_avg': {'label': '平均DSCR要求', 'default': 1.5, 'min': 1.2, 'max': 2.5, 'unit': ''},
                'loan_rate': {'label': '贷款利率', 'default': 4.5, 'min': 2, 'max': 10, 'unit': '%'},
                'loan_years': {'label': '贷款年限', 'default': 15, 'min': 5, 'max': 25, 'unit': '年'},
                'grace_period': {'label': '宽限期', 'default': 2, 'min': 0, 'max': 5, 'unit': '年'},
            ...
            'category': 'financial',
            'is_builtin': True,
            'description': '偿债覆盖倍率（DSCR）衡量项目每年可用于还本付息的现金是否充足，是银行和金融机构放贷的核心审批指标。分子为EBITDA减所得税后的可偿债现金流，分母为当年应付本息。\n\n【中东市场标准】：\n- 最低DSCR：1.25-1.30倍（ADIB、迪拜伊斯兰银行等中东银行标准）\n- 平均DSCR：1.4-1.6倍（项目整体要求）\n- 贷款利率：3.5-5.0%（Masdar项目常获得主权基金担保的优惠利率）\n- 贷款年限：15-20年（比国内项目更长）\n- 宽限期：2-3年（建设期+首年运营期无需还本）\n\n【SPPC沙特标准】：\n- 最低DSCR：1.30倍（沙特公共投资基金PIF支持项目）\n- 宽限期：3年（考虑到沙特储能项目建设期较长）\n\n若某年DSCR < 1.0则项目无法自行偿债，需股东追加担保或调整融资结构。',
        },
        {
            ...
            'parameters': {
                'discount_rate': {'label': '折现率(动态)', 'default': 7, 'min': 4, 'max': 15, 'unit': '%'},
            ...
            'category': 'financial',
            'is_builtin': True,
            'description': '投资回收期是累计净现金流首次由负转正的时间点。静态回收期不考虑资金时间价值，直接累加名义现金流；动态回收期将所有现金流按WACC折现后再累加。\n\n【中东Masdar标准】：\n- 静态回收期：7-10年（得益于较高的电价价差和容量补偿）\n- 动态回收期：9-13年（WACC=6-8%）\n- 目标回收年限：8-10年（中东投资者偏好更短回收期）\n\n【SPPC沙特标准】：\n- 静态回收期：8-11年\n- 动态回收期：10-14年\n\n中东地区由于电价价差较大（峰谷价差可达0.15-0.30美元/kWh），储能项目回收期普遍短于国内项目。回收期越短，项目抗政策/电价波动风险能力越强。',
        },
        {
            ...
            'parameters': {
                'off_peak_price': {'label': '低谷购电价', 'default': 200, 'min': 50, 'max': 500, 'unit': '元/MWh'},
                'peak_price': {'label': '高峰售电价', 'default': 600, 'min': 200, 'max': 1200, 'unit': '元/MWh'},
                'spread_capture': {'label': '价差捕获率', 'default': 85, 'min': 60, 'max': 95, 'unit': '%'},
                'calendar_days': {'label': '日历可用天数', 'default': 330, 'min': 300, 'max': 365, 'unit': '天'},
                'capacity_price': {'label': '容量市场单价', 'default': 50000, 'min': 0, 'max': 200000, 'unit': '元/MW·年'},
                'ancillary_price': {'label': '辅助服务单价', 'default': 30000, 'min': 0, 'max': 100000, 'unit': '元/MW·年'},
                'price_escalation': {'label': '电价年涨幅', 'default': 1.5, 'min': 0, 'max': 5, 'unit': '%/年'},
            ...
            'category': 'financial',
            'is_builtin': True,
            'description': '大型储能电站叠加多个收入来源以提升项目IRR。三收入栈：1）电能量套利——低谷购电/高峰售电，价差捕获率85%考虑了充放电效率和辅助损耗；2）容量市场——按可用装机容量获得固定补偿；3）辅助服务——调频/备用服务的额外收入。\n\n【中东Masdar标准】：\n- 峰谷价差：0.15-0.30美元/kWh（约1.0-2.0元/kWh，中东电价价差全球领先）\n- 容量补偿：30000-80000元/MW·年（ADGM市场）\n- 辅助服务：20000-50000元/MW·年（调频/旋转备用）\n- 价差捕获率：82-88%（高温环境下效率略低）\n- 可用天数：330-350天（高温需降载运行）\n\n【SPPC沙特标准】：\n- 峰谷价差：0.12-0.25美元/kWh\n- 容量补偿：基于沙特电力市场TISO机制\n\n不同市场结构下各收入占比差异显著（套利40-70%、容量20-40%、辅助10-20%）。电价年涨幅1.5-2%为保守通胀估计。',
        },
        ...
        },
        {
            ...
            'mathematical_form': 't_charge = E_dc_nom / ((P_poc × η_ac - P_aux) × η_pcs_charge × √RTE)\nt_discharge = E_dc_nom × √RTE / (P_poc / (η_ac × η_pcs_discharge) + P_aux)',
            'formula_expression': 'charge: dcNomEnergy / ((pocPower * acEfficiency - auxRunPower) * pcsChargeEff * sqrt(dcRte))\ndischarge: dcNomEnergy * sqrt(dcRte) / (pocPower / (acEfficiency * pcsDischargeEff) + auxRunPower)',
            'parameters': {
                'dc_nom_energy': {'label': '电池标称直流容量', 'default': 5, 'min': 0.1, 'max': 100, 'unit': 'MWh'},
                'dc_rte': {'label': 'DC往返效率', 'default': 0.94, 'min': 0.85, 'max': 0.99, 'unit': ''},
                'ac_efficiency': {'label': '交流效率(变压器及线损)', 'default': 98, 'min': 95, 'max': 99.5, 'unit': '%'},
