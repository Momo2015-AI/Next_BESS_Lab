...
def get_builtin_algorithms():
    ...
    return [
        ...
        },
        {
            ...
            'mathematical_form': 't_charge = E_dc_nom / ((P_poc × η_tr - P_aux) × η_pcs_charge)\nt_discharge = E_dc_usable / (P_poc / (η_tr × η_pcs_discharge) + P_aux)',
            'formula_expression': 'charge: dcNomEnergy / ((pocPower * transformerEff - auxRunPower) * pcsChargeEff)\ndischarge: dcUsableEnergy / (pocPower / (transformerEff * pcsDischargeEff) + auxRunPower)',
            'parameters': {
                'dc_nom_energy': {'label': '电池标称直流容量', 'default': 5, 'min': 0.1, 'max': 100, 'unit': 'MWh'},
                'dc_usable_energy': {'label': '电池实际可用能量', 'default': 4.5, 'min': 0.1, 'max': 100, 'unit': 'MWh'},
                'transformer_eff': {'label': '变压器及线损效率', 'default': 98, 'min': 95, 'max': 99.5, 'unit': '%'},
            ...
            'accuracy_level': 'high',
            'accuracy_desc': '工业级精确计算',
            'category': 'engineering',
            'is_builtin': True,
            'description': '储能系统充放电时间工业级精确计算公式。\n\n【充电时间】= 电池标称直流容量 / ((PoC功率 × 变压器及线损效率 - 运行辅助功率) × PCS充电效率)\n物理原理：充电是"自外而内"过程。电能从电网(PoC点)出发，先经变压器及线损打折，再在交流母线上减去辅耗(空调/液冷等)，最后通过PCS变为直流电灌入电池。\n\n【放电时间】= 电池实际可用能量 / (PoC功率 / (变压器及线损效率 × PCS放电效率) + 运行辅助功率)\n物理原理：放电是"自内而外"过程。电池直流端需超额付出代价：先倒推PCS+变压器损耗(功率放大)，再叠加辅耗(电池自供空调散热)。辅耗在直流侧独立相加，与交流侧倒推放大完全隔离。\n\n关键修正：辅耗不参与PCS效率计算，括号优先级保证物理顺序正确。',
        },
