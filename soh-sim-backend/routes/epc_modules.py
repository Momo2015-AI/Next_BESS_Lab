...
def calculate_ipp():
    ...
    if debt_rate > 0 and debt_tenor > 0:
        annual_debt_service = (
            debt_amount * (debt_rate * (1 + debt_rate) ** debt_tenor) / ((1 + debt_rate) ** debt_tenor - 1)
        )
    else:
...
def _auto_match_requirement(req, project_data):
    ...
    if "LVRT" in text:
        ...
        return (
            ("compliant" if gc.get("lvrt_pass") else "partial", "系统支持LVRT功能") if gc else ("N/A", "待电网合规分析")
        )
    if "HVRT" in text:
        gc = project_data.get("grid_compliance", {})
        return (
            ("compliant" if gc.get("hvrt_pass") else "partial", "系统支持HVRT功能") if gc else ("N/A", "待电网合规分析")
        )
...
def generate_bid_document():
    ...
    for ch_template in template["chapters"]:
        chapter = {"num": ch_template["num"], "title": ch_template["title"], "sections": []}
    ...
    return jsonify({"success": True, "data": {"chapters": chapters, "sources": sources}, "id": bd.id})
...
def _generate_section_content(section_template, data):
    ...
    if source == "survey":
        return (
            f"项目名称: {data.get('project_name', 'N/A')}\n"
        ...
        return (
            f"系统架构类型: {data.get('architecture_type', 'N/A')}\n"
        ...
        return (
            f"总条款: {data.get('total_items', 'N/A')}\n"
