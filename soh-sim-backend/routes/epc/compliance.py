...
from services.epc.compliance import (
...
@compliance_bp.route(
    "/api/compliance-matrix/templates", methods=["GET"]
)
@token_required
...
@compliance_bp.route(
    "/api/compliance-matrix/generate", methods=["POST"]
)
@token_required
...
@compliance_bp.route(
    "/api/compliance-matrix/<cm_id>", methods=["GET"]
)
@token_required
...
@compliance_bp.route(
    "/api/compliance-matrix/<cm_id>", methods=["PUT"]
)
@token_required
def update_compliance_matrix_item(cm_id):
    ...
    for item in matrix:
        if item["section"] == data.get("section"):
            item["compliance_status"] = data.get(
                "compliance_status", item["compliance_status"]
            )
            item["response"] = data.get("response", item["response"])
            item["evidence"] = data.get(
                "evidence", item.get("evidence", "")
            )
            item["reference_doc"] = data.get(
                "reference_doc", item.get("reference_doc", "")
            )
            item["verified"] = data.get(
                "verified", item.get("verified", False)
            )
            break

    obj.matrix_data = json.dumps(matrix)
    obj.compliant_count = sum(
        1
        for m in matrix
        if m["compliance_status"] == "compliant"
    )
    obj.non_compliant_count = sum(
        1
        for m in matrix
        if m["compliance_status"] == "non_compliant"
    )
    obj.partial_count = sum(
        1 for m in matrix if m["compliance_status"] == "partial"
    )
    db.session.commit()
