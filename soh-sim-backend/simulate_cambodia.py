...
years = 26
soh_custom = [
    100.0, 98.5, 97.1, 95.8, 94.6, 93.5, 92.4, 91.4, 90.4, 89.5,
    88.6, 87.8, 87.0, 86.2, 85.4, 84.7, 84.0, 83.3, 82.6, 82.0,
    81.3, 80.7, 80.1, 79.5, 78.9, 78.3
]
# RTE @ AC side (已含AC损耗), 招标≥85%
rte_custom = [
    86.5, 86.4, 86.3, 86.2, 86.1, 86.0, 85.9, 85.8, 85.7, 85.6,
    85.5, 85.4, 85.3, 85.2, 85.1, 85.0, 85.0, 85.0, 85.0, 85.0,
...
print("=" * 80)
print(f"{'Cycle':>6} | {'Year':>4} | {'SoH (%)':>8} | {'RTE (%)':>8} | {'Usable (MWh)':>13} | {'SOH>76%':>8} | {'RTE≥85%':>8}")
print("-" * 80)
...
for i in range(24):
    ...
    soh_ok = s >= 76.0
    rte_ok = r >= 85.0
    if not soh_ok: all_soh_ok = False
    if not rte_ok: all_rte_ok = False
    print(f"{cycle:>6} | {i:>4} | {s:>8.2f} | {r:>8.2f} | {u:>13.2f} |  {'✅' if soh_ok else '❌'}   |  {'✅' if rte_ok else '❌'}")
...
fin = calculate_full_financial(total_ac_usable, {
    "_capexInternal": {
...
m = fin["metrics"]
for k, label in [("projectIrr","项目 IRR"),("equityIrr","权益 IRR"),("npv","NPV @7%"),("lcos","LCOS"),("payback","回收期"),("roi","ROI")]:
    v = m[k]
    if v is None: disp = "N/A"
    elif k in ("npv",): disp = f"${v:,.0f}"
    elif k in ("lcos",): disp = f"${v:.4f}/kWh"
    elif k in ("payback",): disp = f"{v:.1f} 年"
    else: disp = f"{v:.2f}%"
...
for ct in ["forced-air", "liquid", "SiC-liquid"]:
    ...
    print(f"  {ct:>14} | {c['cooling_power_kw']:>8.2f} | {c['cooling_power_kw']+FIXED_AUX_KW:>9.2f} | {c['cop']:>5.1f} | {annual:>10.0f}")
