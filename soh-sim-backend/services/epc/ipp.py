...
def calculate_ipp_service(data):
    ...
    capex = float(
        data.get("total_capex_usd", 500_000_000)
    )
    capacity_mw = float(data.get("capacity_mw", 100))
    energy_mwh = float(data.get("energy_mwh", 200))

    cap_price = float(
        data.get("capacity_price_usd_kw_month", 8.0)
    )
    energy_price = float(
        data.get("energy_price_usd_kwh", 0.05)
    )
    escalation = float(
        data.get("ppa_escalation_rate", 0.02)
    )
    avail_guarantee = float(
        data.get("availability_guarantee", 0.98)
    )
    avail_penalty = float(
        data.get("availability_penalty_usd_kw", 5.0)
    )
    rte_guarantee = float(
        data.get("rte_guarantee", 90.0)
    )
    rte_initial = float(data.get("rte_initial", 92.0))
    rte_decay = float(
        data.get("rte_decay_rate", 0.3)
    )
    ...
    debt_rate = float(
        data.get("debt_interest_rate", 0.05)
    )
    debt_tenor = int(
        data.get("debt_tenor_years", 15)
    )
    discount_rate = float(
        data.get("discount_rate", 0.08)
    )
    annual_opex = float(
        data.get("annual_opex_usd", 5_000_000)
    )
    insurance_rate = float(
        data.get("insurance_rate", 0.005)
    )
    land_lease = float(
        data.get("land_lease_usd_year", 500_000)
    )
    ...
    if debt_rate > 0 and debt_tenor > 0:
        annual_debt_service = (
            ...
            * (
                debt_rate
                * (1 + debt_rate) ** debt_tenor
            )
            / ((1 + debt_rate) ** debt_tenor - 1)
        )
    else:
        annual_debt_service = (
            debt_amount / debt_tenor
            if debt_tenor > 0
            else 0
        )
    ...
    cycles_per_year = 300
    annual_energy_kwh = (
        energy_mwh * 1000 * cycles_per_year
    )
    ...
    for y in range(years):
        esc = (1 + escalation) ** y
        cap_revenue = (
            capacity_mw * 1000 * cap_price * 12 * esc
        )
        ...
        perf_penalty = (
            energy_revenue * perf_penalty_rate
            if actual_rte < rte_guarantee
            else 0
        )
        total_revenue = (
            cap_revenue
        ...
        opex_total = (
            (annual_opex + capex * insurance_rate + land_lease)
            * (1.02 ** y)
        )
    ...
    dscr_avg = (
        sum(dscrs) / len(dscrs) if dscrs else 0
    )
        ...
        cashflow_detail.append(
            {
                "year": y,
                "revenue": (
                    round(project_cf[y + 1], 0)
                    if y + 1 < len(project_cf)
                    else 0
                ),
                "cumulative": round(
                    sum(project_cf[: y + 2]), 0
                ),
            }
        )
