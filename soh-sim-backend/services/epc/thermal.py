...
def calculate_thermal_service(data):
    ...
    cell_ah = float(
        data.get("cell_capacity_ah", 280)
    )
    cell_resistance = float(
        data.get("cell_resistance_ohm", 0.00025)
    )
    c_rate = float(data.get("c_rate", 0.5))
    cells_per_container = int(
        data.get("cells_per_container", 5000)
    )
    cooling_type = data.get(
        "cooling_type", "liquid"
    )
    ...
    heat_infiltration = (
        container_area
        * u_value
        * (env_temp - target_temp)
    )
    total_cooling_kw = (
        (total_heat_kw + heat_infiltration / 1000)
        * 1.2
    )
    ...
    return {
        ...
        "coolant_flow_rate_lpm": round(
            coolant_flow_lpm, 1
        ),
        "hvac_capacity_kw": round(
            total_cooling_kw, 2
        ),
        "hvac_cop": 2.5,
        "hvac_redundancy": 1,
        "cell_heat_generation_w": round(
            heat_per_cell, 3
        ),
        ...
        "annual_cooling_energy_kwh": round(
            annual_cooling, 0
        ),
