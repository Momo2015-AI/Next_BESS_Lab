...
def validate_aux_power_params(params):
    """校验辅助功耗计算参数，返回 (is_valid, error_message)"""
    days = params.get("days", 365)
    cycles = params.get("cycles", 2)
    hours = params.get("hours", 2)
    cap = params.get("cap", 5)
    units = params.get("units", 62)
    dcRte = params.get("dcRte", 0.941)
    pcsEff = params.get("pcsEff", 0.987)
    acEff = params.get("acEff", 0.975)
    bRun = params.get("bRun", 20)
    bStd = params.get("bStd", 4)
    pRun = params.get("pRun", 5)
    pStd = params.get("pStd", 1.5)
    pStation = params.get("pStation", 7.2)

    if days < 0 or cycles < 0 or hours < 0:
        return False, "天数、循环次数、时长不能为负值"
    if cap < 0 or units < 0:
        return False, "容量和台数不能为负值"
    if not (0 < dcRte <= 1) or not (0 < pcsEff <= 1) or not (0 < acEff <= 1):
        return False, "效率值必须在 (0, 1] 范围内"
    if bRun < 0 or bStd < 0 or pRun < 0 or pStd < 0 or pStation < 0:
        return False, "功率参数不能为负值"

    return True, None


def calculate_aux_power(params):
    """
    计算辅助功耗
    ...
    cycles = float(params.get("cycles", 2))
    hours = float(params.get("hours", 2))
    cap = float(params.get("cap", 5))
    units = float(params.get("units", 62))
    dcRte = float(params.get("dcRte", 0.941))
    pcsEff = float(params.get("pcsEff", 0.987))
    acEff = float(params.get("acEff", 0.975))
    bRun = float(params.get("bRun", 20))
    bStd = float(params.get("bStd", 4))
    pRun = float(params.get("pRun", 5))
    pStd = float(params.get("pStd", 1.5))
    pStation = float(params.get("pStation", 7.2))
