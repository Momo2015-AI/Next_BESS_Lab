...
class TestDesignEngine:
    ...
    def test_solution_has_complete_fields(self, client, auth_headers_eng):
        ...
        required_fields = ["container", "pcs", "containerQty", "pcsQty",
                          "estimatedCapex", "efficiencyChain", "degradationModel", "auxPower"]
    ...
    def test_different_strategies(self, client, auth_headers_eng):
        """2.3 不同策略产生不同排序"""
        resp_economic = _post(client, "/api/design/auto", {
            "survey_params": SURVEY_PARAMS,
            "strategy": "economic",
        }, auth_headers_eng)
        resp_balanced = _post(client, "/api/design/auto", {
            "survey_params": SURVEY_PARAMS,
            "strategy": "balanced",
        }, auth_headers_eng)
...
class TestSimulationEngine:
    ...
    def test_energy_accounting_fields(self, client, auth_headers_eng, sim_body):
        ...
        resp = _post(client, "/api/simulation/run", sim_body, auth_headers_eng)
        data = _assert_success(resp)
        energy_fields = ["initGross", "initAux", "initAcUsable",
        ...
        if isinstance(comparison, dict):
            ...
            for c in comparison:
                if isinstance(c, dict):
                    assert "strategy" in c or "label" in c or "key" in c, \
                        f"Missing strategy identifier in {c}"
        else:
    ...
    def fin_body(self, simulation_output):
        ...
        return {
            ...
            "design_output": {
                ...
                "pcsQty": 10,
                "duration": 2,
                "estimatedCapex": {"totalCapex": 50000000, "equipmentCost": 35000000,
                                  "epcCost": 10000000, "developmentCost": 5000000},
            },
...
class TestEnginePipeline:
    ...
    def test_full_pipeline_design_sim_fin(self, client, auth_headers_eng):
        ...
        # Design
        d_resp = _post(client, "/api/design/auto", {
            "survey_params": SURVEY_PARAMS,
        }, auth_headers_eng)
        ...
        # Simulation
        s_resp = _post(client, "/api/simulation/run", {
            "design_output": {
        ...
        # Financial
        f_resp = _post(client, "/api/financial/calculate", {
            "simulation_output": {
        ...
        assert f_data["metrics"]["npv"] != 0 or f_data["metrics"]["projectIrr"] != 0, \
            "Financial metrics are all zero"
    ...
    def test_arrays_consistent_length_26(self, client, auth_headers_eng):
        ...
        s_resp = _post(client, "/api/simulation/run", {
            "design_output": {
                "container": sol["container"], "pcs": sol["pcs"],
                "containerQty": sol["containerQty"], "pcsQty": sol["pcsQty"],
                "duration": sol.get("duration", 2),
            },
            "survey_params": SURVEY_PARAMS,
        }, auth_headers_eng)
        ...
        # 验证所有仿真数组
        array_keys = ["soh", "rte", "dod", "augQty", "totalAcUsable", "meetsReq",
                      "initGross", "initAcUsable", "augAcUsable"]
...
class TestOrchestrator:
    ...
    def test_different_target_metrics(self, client, auth_headers_eng):
        ...
        resp_lcos = _post(client, "/api/workflow/full", {
            "survey_params": SURVEY_PARAMS,
            "target_metric": "lcos",
        }, auth_headers_eng)
        resp_irr = _post(client, "/api/workflow/full", {
            "survey_params": SURVEY_PARAMS,
            "target_metric": "irr",
        }, auth_headers_eng)
...
class TestBusinessModules:
    ...
    def test_boq_save_and_load(self, client, auth_headers_eng):
        ...
        # 先保存
        boq_items = [
            {"sectionCode": "B01", "seq": 1, "name": "Battery Container",
             "spec": "5MWh", "unit": "set", "quantity": 10,
             "unitPrice": 500000, "totalPrice": 5000000, "version": 1},
        ]
        resp = _post(client, "/api/boq/items", {
    ...
    def test_capex_from_boq(self, client, auth_headers_eng):
        ...
        resp = _post(client, "/api/financial/capex-from-boq", {
            "boqItems": boq_items,
        }, auth_headers_eng)
        data = _assert_success(resp)
        assert "equipment" in data or "totalCapex" in data or "total" in data, \
            "CAPEX aggregation response invalid"
    ...
    def test_version_lifecycle(self, client, auth_headers_eng):
        ...
        # 对比版本（需要两个版本，创建第二个）
        ver2_resp = _post(client, f"/api/projects/{project_id}/versions", {
            "name": "v2.0",
        ...
        # 对比
        cmp_resp = _post(client, "/api/versions/compare", {
    ...
    def test_report_export(self, client, auth_headers_eng):
        """7.8 报告导出"""
        resp = _post(client, "/api/report/technical", {
        ...
        assert resp.status_code in (200, 400, 404, 500), \
            f"Report export unexpected status: {resp.status_code}"
    ...
    def test_epc_architecture(self, client, auth_headers_eng):
        """7.10a EPC - 系统架构设计"""
        resp = _post(client, "/api/system-architecture/design", {
            "totalPower": 50, "ratedEnergy": 100, "duration": 2,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500), \
            f"EPC architecture unexpected: {resp.status_code}"

    def test_epc_safety(self, client, auth_headers_eng):
        """7.10b EPC - 安全设计"""
        resp = _post(client, "/api/safety-design/analyze", {
            "totalPower": 50, "ratedEnergy": 100,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)

    def test_epc_thermal(self, client, auth_headers_eng):
        """7.10c EPC - 热管理"""
        resp = _post(client, "/api/thermal-management/calculate", {
            "totalPower": 50, "ratedEnergy": 100,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
    ...
    def test_epc_grid_compliance(self, client, auth_headers_eng):
        """7.10d EPC - 电网合规"""
        resp = _post(client, "/api/grid-compliance/analyze", {
            "totalPower": 50, "ratedEnergy": 100,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
    ...
    def test_epc_ipp_financial(self, client, auth_headers_eng):
        """7.10e EPC - IPP 财务模型"""
        resp = _post(client, "/api/ipp-financial/calculate", {
            "totalPower": 50, "ratedEnergy": 100,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
    ...
    def test_epc_compliance_matrix(self, client, auth_headers_eng):
        """7.10f EPC - 合规矩阵"""
        resp = _post(client, "/api/compliance-matrix/generate", {
            "totalPower": 50,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
    ...
    def test_epc_scada(self, client, auth_headers_eng):
        ...
        resp = _post(client, "/api/scada-ems/design", {
            "totalPower": 50,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
    ...
    def test_epc_hv(self, client, auth_headers_eng):
        """7.10h EPC - 高压互联"""
        resp = _post(client, "/api/hv-interconnection/design", {
            "totalPower": 50, "ratedEnergy": 100,
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
    ...
    def test_epc_bid_document(self, client, auth_headers_eng):
        """7.10i EPC - 投标文档生成"""
        resp = _post(client, "/api/bid-document/generate", {
            "projectName": "Test",
        }, auth_headers_eng)
        assert resp.status_code in (200, 201, 400, 404, 500)
...
class TestRBAC:
    ...
    def test_engineer_cannot_access_users(self, client, auth_headers_eng):
        ...
        assert resp.status_code == 403, \
            f"Engineer should be forbidden, got {resp.status_code}"
    ...
    def test_admin_can_modify_permissions(self, client, auth_headers_admin):
        """8.4 Admin 可修改角色权限"""
        resp = _put(client, "/api/rbac/role-permissions/solution_engineer", {
