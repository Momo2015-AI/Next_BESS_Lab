...
class TestEfficiencyFactorsNull:
    ...
    def test_null_factors_uses_custom_rte(self):
        ...
        from services.pipeline import calculate_energy_accounting, FACTOR_DEFAULTS
        ...
        result_null = calculate_energy_accounting(
            params, soh, rte, dod, aug_qty, efficiency_factors=None
        )

        # With None, cycle_rte = c_rte * ac_efficiency = (86.5/100) * 1.0 = 0.865
        expected_cycle_rte = 0.865
        expected_init_gross_year0 = params["ratedEnergy"] * params["initContainerQty"] * (dod[0] / 100) * expected_cycle_rte * (soh[0] / 100)
        # 5 * 10 * 0.9 * 0.865 * 1.0 = 38.925
        assert abs(result_null["initGross"][0] - expected_init_gross_year0) < 0.01, \
        ...
        result_default = calculate_energy_accounting(
            params, soh, rte, dod, aug_qty, efficiency_factors=FACTOR_DEFAULTS
        )
        assert abs(result_null["initGross"][0] - result_default["initGross"][0]) > 0.01, \
    ...
    def test_default_factors_uses_chain(self):
        ...
        assert result_default["initGross"][0] != result_null["initGross"][0], \
    ...
    def test_null_factors_with_different_rte_per_year(self):
        ...
        for i in range(26):
            ...
            expected_gross = params["ratedEnergy"] * params["initContainerQty"] * (dod[i] / 100) * expected_rte * (soh[i] / 100)
            assert abs(result["initGross"][i] - expected_gross) < 0.02, \
                f"Year {i}: expected initGross ~ {expected_gross}, got {result['initGross'][i]}"

        # Verify monotonic decrease
        for i in range(1, 26):
            assert result["initGross"][i] <= result["initGross"][i - 1], \
    ...
    def test_calculate_full_pipeline_null_factors(self):
        ...
        from services.pipeline import calculate_full_pipeline, NUM_YEARS
        ...
        assert result["efficiencyCurves"] is None, \
            "efficiencyCurves should be None when efficiencyFactors is None"
        assert result["efficiencyDetail"] is None, \
            "efficiencyDetail should be None when efficiencyFactors is None"

        # rte should match custom values (since no chain overrides them)
        for i in range(NUM_YEARS):
            assert abs(result["rte"][i] - custom_rte[i]) < 0.01, \
    ...
    def test_calculate_full_pipeline_default_factors(self):
        ...
        comes from chain."""
        from services.pipeline import calculate_full_pipeline, FACTOR_DEFAULTS, NUM_YEARS
        ...
        assert result["efficiencyCurves"] is not None, \
            "efficiencyCurves should not be None with default factors"
        assert result["efficiencyDetail"] is not None, \
            "efficiencyDetail should not be None with default factors"
    ...
    def test_null_factors_ac_efficiency_still_applied(self):
        ...
        assert abs(result["initGross"][0] - expected_gross) < 0.01, \
        ...
        result2 = calculate_energy_accounting(
            params2, soh, rte, dod, aug_qty, efficiency_factors=None
        )
        # cycle_rte = 0.85 * 0.95 = 0.8075
        expected_gross2 = 5 * 10 * 0.9 * 0.8075 * 1.0
        assert abs(result2["initGross"][0] - expected_gross2) < 0.01, \
            f"Expected initGross[0] ~ {expected_gross2}, got {result2['initGross'][0]}"
        assert result2["initGross"][0] < result["initGross"][0], \
            "Lower acEfficiency should produce lower initGross"
    ...
    def test_simulation_engine_null_factors_via_api(self, client, auth_headers_eng):
        ...
        assert data.get("efficiencyCurves") is None, \
            "efficiencyCurves should be None via API with null factors"
        ...
        for i in range(26):
            assert abs(result_rte[i] - custom_rte[i]) < 0.01, \
    ...
    def test_missing_key_vs_explicit_none(self):
        ...
        assert result_absent["efficiencyCurves"] is not None, \
        ...
        assert result_none["efficiencyCurves"] is None, \
            "When key is explicitly None, efficiencyCurves should be None (custom RTE)"

        # The results should differ
        assert result_absent["initGross"][0] != result_none["initGross"][0], \
        ...
        assert result_absent["initGross"][0] == result_defaults["initGross"][0], \
