...
| 维度 | 现状 |
|------|------|
| 仿真引擎 | ✅ 25 年容量矩阵 + 补容（Augmentation）逻辑，前后端各一份等价实现 |
| 财务引擎 | ✅ 前端有 IRR/NPV/现金流/敏感性分析（部分指标占位未实现） |
...
| **项目管理** | ❌ 无"项目"概念 |
| **报告导出** | ❌ 无 PDF/导出能力 |
| **已知缺陷** | `App.vue:156` `fetchCalculation` 语法结构错乱；`app.py:15` 硬编码 Linux 上传路径 |
...
| ID | 需求 | 优先级 |
|----|------|--------|
| FR-1 | 持久化层：引入 SQLite + SQLAlchemy，项目/调研/配置/结果/报告入库 | Must |
| FR-2 | **调研表作为项目入口**：新建项目即填调研表（复用现有 `RunningConditions.vue`），数据入库绑定项目 | Must |
| FR-3 | 项目管理：创建/列表/打开/删除项目，自动生成 UUID | Must |
| FR-4 | 保存仿真：将当前 params + soh/rte/dod/augQty + 计算结果保存到指定项目 | Must |
| FR-5 | 加载仿真：从数据库恢复某项目的完整配置与结果到前端 | Must |
| FR-6 | 技术报告 PDF 导出（含系统配置 + 25 年容量表） | Must |
| FR-7 | 设备清单 BOM PDF 导出（基于 products.json 的选型） | Must |
| FR-8 | 修复 `App.vue` 的 `fetchCalculation` 语法 bug | Must |
| FR-9 | 修复后端 `UPLOAD_DIR` 硬编码路径（Windows 兼容） | Must |
| FR-10 | 保留并接入调研表的"投标文件导入自动填充"功能（现有 `/api/upload/extract`），抽取结果可入库 | Must |
...
| 步骤 | 内容 | 验证方式 |
|------|------|----------|
| **S1** | 修 `App.vue` 的 `fetchCalculation` 语法 bug | 前端能正常 build，点"执行仿真计算"不报错 |
| **S2** | 后端模块化拆分（routes/services/models/db/config），现有 4 个接口行为不变 | 跑现有接口，响应与拆分前一致 |
| **S3** | 修 `UPLOAD_DIR` 硬编码路径 | Windows 下上传文件不再报路径错 |
| **S4** | 数据模型 + DB 初始化（4 张表） | 启动时自动建表，可手动查表 |
| **S5** | 项目 CRUD 接口 + service | 用 curl/Postman 验证增删改查 |
| **S6** | 保存/加载仿真（PUT/GET simulation） | 创建项目→保存→重新GET→数据一致 |
| **S7** | ReportLab 报告生成（technical + bom） | 接口返回合法 PDF，中文正常 |
| **S8** | 前端接项目列表/保存/导出 | 完整跑通 UC-1~UC-4 |
...
| `calculate` 逻辑前后端复制两份 | 改一处忘改另一处 | 第一波不动算法逻辑；中长期把前端本地计算改为纯调后端（第二波讨论） |
| SQLite 并发写 | 多人同时保存冲突 | 本期单用户无此问题；多租户阶段迁移 PostgreSQL 时解决 |
...
- [ ] **数据模型**：4 张表是否覆盖需求？字段是否够用？`tenant_id` 预留是否认可？
- [ ] **API**：7 个新增接口是否够用？命名/风格是否可接受？
