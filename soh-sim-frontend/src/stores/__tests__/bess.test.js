...
describe('bess store', () => {
  ...
  describe('deprecated runPipeline()', () => {
    ...
    it('calls POST /api/pipeline/calculate and updates store', async () => {
      const store = useBessStore()
      ...
      expect(post).toHaveBeenCalledWith('/api/pipeline/calculate', expect.any(Object))
      expect(store.calculating).toBe(false)
      expect(store.degradation.soh[0]).toBe(95)
      expect(store.financial.metrics.npv).toBe(1000000)
    })

    it('sets calculationError on failure', async () => {
      const store = useBessStore()

      post.mockRejectedValueOnce(new Error('Pipeline error'))

      await store.runPipeline()

      expect(store.calculating).toBe(false)
      expect(store.calculationError).toBe('Pipeline error')
    })
  })
})
