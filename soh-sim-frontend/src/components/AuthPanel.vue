<template>
  <div class="auth-panel h-full flex items-center justify-center p-4">
    <div class="w-full max-w-md">
      <!-- 登录/注册表单 -->
      <div class="rounded-xl p-6 shadow-2xl" style="background-color: var(--color-card); border: 1px solid var(--color-border);">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div v-if="!isLogin" class="flex items-start gap-2">
            <input type="checkbox" v-model="agreedToTerms" required 
              style="accent-color: var(--color-accent-secondary);" class="mt-0.5">
            <span class="text-xs" style="color: var(--color-text-muted);">
              我已阅读并同意 <a href="#" style="color: var(--color-accent-secondary);" onmouseover="this.style.textDecoration='underline';" onmouseout="this.style.textDecoration='none';">服务条款</a> 和 <a href="#" style="color: var(--color-accent-secondary);" onmouseover="this.style.textDecoration='underline';" onmouseout="this.style.textDecoration='none';">隐私政策</a>
            </span>
          </div>
          ...
          <button type="submit" :disabled="loading || (!isLogin && !agreedToTerms)"
            ...
            :style="loading || (!isLogin && !agreedToTerms) ? { backgroundColor: 'var(--color-border)', color: 'var(--color-text-muted)' } : { backgroundColor: 'var(--color-accent-secondary)', color: 'white' }"
          ...
          <div class="text-center">
            <button type="button" @click="isLogin = !isLogin; clearForm()"
              class="text-xs" style="color: var(--color-accent-secondary);"
        ...
        <!-- 错误提示 -->
        <div v-if="errorMessage" class="mt-4 p-3 rounded-lg" style="background-color: var(--color-danger-glow); border: 1px solid var(--color-danger);">
          <p class="text-xs" style="color: var(--color-danger);">{{ errorMessage }}</p>
        </div>
      </div>

      <!-- 快捷登录（演示用） -->
      <div class="mt-4 p-3 rounded-lg" style="background-color: var(--color-card-dark); border: 1px solid var(--color-border);">
        <div class="text-xs text-center mb-2" style="color: var(--color-text-muted);">快捷体验（无需注册）</div>
        <div class="flex gap-2">
          <button @click="quickLogin('admin')" class="flex-1 text-xs py-1.5 rounded transition-colors"
          ...
          <button @click="quickLogin('engineer')" class="flex-1 text-xs py-1.5 rounded transition-colors"
            style="background-color: var(--color-card); border: 1px solid var(--color-border); color: var(--color-text-secondary);"
            onmouseover="this.style.borderColor='var(--color-accent)';"
            onmouseout="this.style.borderColor='var(--color-border)';">
            仿真工程师
          </button>
          <button @click="quickLogin('guest')" class="flex-1 text-xs py-1.5 rounded transition-colors"
            style="background-color: var(--color-card); border: 1px solid var(--color-border); color: var(--color-text-secondary);"
            onmouseover="this.style.borderColor='var(--color-accent)';"
            onmouseout="this.style.borderColor='var(--color-border)';">
            访客
