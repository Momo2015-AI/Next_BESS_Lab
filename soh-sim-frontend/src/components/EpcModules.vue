<template>
  <div class="space-y-4">
    <!-- 模块标签页 -->
    <div class="flex flex-wrap gap-2 border-b" style="border-color: #E0E0E0;">
      <button v-for="tab in tabs" :key="tab.id"
        @click="activeModule = tab.id"
        class="px-4 py-2 text-sm font-medium border-b-2 transition-all"
        :style="activeModule === tab.id ? { borderColor: '#2F5496', color: '#2F5496' } : { borderColor: 'transparent', color: '#666' }">
        {{ tab.icon }} {{ tab.label }}
      </button>
    </div>
    ...
    <div v-show="activeModule === 'architecture'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">系统架构设计</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label class="block text-xs mb-1" style="color: #999;">系统功率 (MW)</label>
            <input v-model.number="archForm.total_power_mw" type="number" class="form-input" placeholder="1400" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">系统能量 (MWh)</label>
            <input v-model.number="archForm.total_energy_mwh" type="number" class="form-input" placeholder="8400" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">架构类型</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">耦合方式</label>
        ...
        <div v-if="archResult" class="mt-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div v-for="item in archResult.topology_data.levels" :key="item.name"
              class="bg-gray-50 rounded-lg p-3 text-center">
              <div class="text-2xl font-bold" style="color: #2F5496;">{{ item.count }}</div>
              <div class="text-xs" style="color: #999;">{{ item.name }} ({{ item.unit }})</div>
            </div>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
            <div class="result-card"><div class="result-value">{{ archResult.pcs_count }}</div><div class="result-label">PCS总数</div></div>
            <div class="result-card"><div class="result-value">{{ archResult.dc_bus_voltage }}V</div><div class="result-label">DC母线电压</div></div>
            <div class="result-card"><div class="result-value">{{ archResult.total_containers }}</div><div class="result-label">集装箱总数</div></div>
            <div class="result-card"><div class="result-value">{{ archResult.stage_count }}</div><div class="result-label">分期数</div></div>
            <div class="result-card"><div class="result-value">{{ archResult.duration_hours }}h</div><div class="result-label">储能时长</div></div>
          </div>
          <div v-if="archResult.stages" class="mt-4">
            <h4 class="text-sm font-bold mb-2" style="color: #666;">分期建设方案</h4>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div v-for="stage in archResult.stages" :key="stage.stage"
                class="bg-blue-50 rounded-lg p-3 border" style="border-color: #BFDBFE;">
                <div class="font-bold text-sm" style="color: #2F5496;">第{{ stage.stage }}期</div>
    ...
    <div v-show="activeModule === 'gridCompliance'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">电网合规分析</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label class="block text-xs mb-1" style="color: #999;">电网标准</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">并网点电压 (kV)</label>
            <input v-model.number="gcForm.grid_voltage_kv" type="number" class="form-input" placeholder="33" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">电网频率 (Hz)</label>
            <input v-model.number="gcForm.grid_frequency_hz" type="number" step="0.01" class="form-input" placeholder="50" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">PCS数量</label>
            <input v-model.number="gcForm.pcs_count" type="number" class="form-input" placeholder="10" />
          </div>
        </div>
        <button @click="analyzeGridCompliance" :disabled="loading"
          class="px-6 py-2 rounded-lg text-white font-medium text-sm" style="background-color: #2F5496;">
          {{ loading ? '分析中...' : '执行合规分析' }}
        </button>

        <div v-if="gcResult" class="mt-6 space-y-4">
          <div class="flex items-center gap-4 p-4 rounded-lg"
            :style="gcResult.overall_pass ? { backgroundColor: '#D1FAE5' } : { backgroundColor: '#FEE2E2' }">
            <span class="text-3xl">{{ gcResult.overall_pass ? '✅' : '❌' }}</span>
            <div>
              <div class="font-bold" :style="gcResult.overall_pass ? { color: '#065F46' } : { color: '#991B1B' }">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value" :class="gcResult.lvrt_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.lvrt_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">LVRT低电压穿越</div></div>
            <div class="result-card"><div class="result-value" :class="gcResult.hvrt_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.hvrt_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">HVRT高电压穿越</div></div>
            <div class="result-card"><div class="result-value" :class="gcResult.freq_response_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.freq_response_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">频率响应</div></div>
            <div class="result-card"><div class="result-value" :class="gcResult.reactive_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.reactive_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">无功功率 ({{ gcResult.reactive_capacity_mvar }}MVar)</div></div>
            <div class="result-card"><div class="result-value" :class="gcResult.power_quality_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.thd }}%</div><div class="result-label">THD (限值5%)</div></div>
            <div class="result-card"><div class="result-value" :class="gcResult.anti_islanding_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.anti_islanding_time_s }}s</div><div class="result-label">防孤岛 (限值2s)</div></div>
            <div class="result-card"><div class="result-value" :class="gcResult.comm_pass ? 'text-green-600' : 'text-red-600'">{{ gcResult.comm_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">通信合规</div></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-show="activeModule === 'safety'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">安全与消防设计</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label class="block text-xs mb-1" style="color: #999;">系统容量 (MWh)</label>
            <input v-model.number="sfForm.system_capacity_mwh" type="number" class="form-input" placeholder="100" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">集装箱数量</label>
            <input v-model.number="sfForm.container_count" type="number" class="form-input" placeholder="20" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">化学体系</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">灭火系统</label>
        ...
        <button @click="analyzeSafety" :disabled="loading"
          class="px-6 py-2 rounded-lg text-white font-medium text-sm" style="background-color: #2F5496;">
          {{ loading ? '分析中...' : '执行安全分析' }}
        </button>
        ...
        <div v-if="sfResult" class="mt-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value">{{ sfResult.zone_count }}</div><div class="result-label">防火分区数</div></div>
            <div class="result-card"><div class="result-value">{{ sfResult.container_spacing_m }}m</div><div class="result-label">集装箱间距</div></div>
            <div class="result-card"><div class="result-value">{{ sfResult.thermal_runaway_temp_c }}°C</div><div class="result-label">热失控温度</div></div>
            <div class="result-card"><div class="result-value">{{ sfResult.propagation_time_min }}min</div><div class="result-label">蔓延时间</div></div>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value" :class="sfResult.ul_9540a_pass ? 'text-green-600' : 'text-red-600'">{{ sfResult.ul_9540a_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">UL 9540A</div></div>
            <div class="result-card"><div class="result-value" :class="sfResult.nfpa_855_pass ? 'text-green-600' : 'text-red-600'">{{ sfResult.nfpa_855_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">NFPA 855</div></div>
            <div class="result-card"><div class="result-value" :class="sfResult.iec_62619_pass ? 'text-green-600' : 'text-red-600'">{{ sfResult.iec_62619_pass ? 'PASS' : 'FAIL' }}</div><div class="result-label">IEC 62619</div></div>
            <div class="result-card"><div class="result-value">{{ sfResult.suppression_capacity_kg }}kg</div><div class="result-label">灭火剂容量</div></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-show="activeModule === 'ipp'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        ...
        <div v-if="ippResult" class="mt-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value" style="color: #2F5496;">${{ formatNum(ippResult.npv_usd) }}</div><div class="result-label">NPV</div></div>
            <div class="result-card"><div class="result-value" style="color: #2F5496;">{{ ippResult.irr }}%</div><div class="result-label">项目IRR</div></div>
            <div class="result-card"><div class="result-value" style="color: #2F5496;">{{ ippResult.equity_irr }}%</div><div class="result-label">股权IRR</div></div>
            <div class="result-card"><div class="result-value" style="color: #2F5496;">${{ ippResult.lcoe_usd_kwh }}/kWh</div><div class="result-label">LCOE</div></div>
          </div>
          <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div class="result-card"><div class="result-value">{{ ippResult.dscr_avg }}</div><div class="result-label">平均DSCR</div></div>
            <div class="result-card"><div class="result-value">{{ ippResult.dscr_min }}</div><div class="result-label">最小DSCR</div></div>
            <div class="result-card"><div class="result-value">{{ ippResult.payback_years }}年</div><div class="result-label">回收期</div></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-show="activeModule === 'matrix'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">合规矩阵生成</h3>
        <div class="flex gap-4 mb-4">
          ...
          <button @click="generateMatrix" :disabled="loading"
            class="px-6 py-2 rounded-lg text-white font-medium text-sm" style="background-color: #2F5496;">
            {{ loading ? '生成中...' : '生成合规矩阵' }}
          </button>
        </div>
        ...
        <div v-if="matrixResult" class="mt-4">
          <div class="grid grid-cols-4 gap-3 mb-4">
            <div class="result-card"><div class="result-value">{{ matrixResult.total }}</div><div class="result-label">总条款</div></div>
            <div class="result-card"><div class="result-value text-green-600">{{ matrixResult.compliant }}</div><div class="result-label">合规</div></div>
            <div class="result-card"><div class="result-value text-red-600">{{ matrixResult.non_compliant }}</div><div class="result-label">不合规</div></div>
            <div class="result-card"><div class="result-value text-yellow-600">{{ matrixResult.partial }}</div><div class="result-label">部分合规</div></div>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              ...
              <tbody>
                <tr v-for="item in matrixResult.matrix" :key="item.section" class="border-b" style="border-color: #E0E0E0;">
    ...
    <div v-show="activeModule === 'thermal'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">热管理设计</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label class="block text-xs mb-1" style="color: #999;">最高环境温度 (°C)</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">充放电倍率 (C)</label>
            <input v-model.number="tmForm.c_rate" type="number" step="0.1" class="form-input" placeholder="0.5" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">冷却方式</label>
        ...
        <button @click="calculateThermal" :disabled="loading"
          class="px-6 py-2 rounded-lg text-white font-medium text-sm" style="background-color: #2F5496;">
          {{ loading ? '计算中...' : '执行热管理计算' }}
        </button>
        ...
        <div v-if="tmResult" class="mt-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value">{{ tmResult.cooling_power_kw }}kW</div><div class="result-label">制冷功率</div></div>
            <div class="result-card"><div class="result-value">{{ tmResult.coolant_flow_rate_lpm }}L/min</div><div class="result-label">冷却液流量</div></div>
            <div class="result-card"><div class="result-value">{{ tmResult.max_cell_temp_c }}°C</div><div class="result-label">最高电芯温度</div></div>
            <div class="result-card"><div class="result-value">{{ formatNum(tmResult.annual_cooling_energy_kwh) }}kWh</div><div class="result-label">年制冷能耗</div></div>
          </div>
          <div v-if="tmResult.derating_curve" class="mt-4">
            <h4 class="text-sm font-bold mb-2" style="color: #666;">高温降额曲线</h4>
            <div class="flex gap-1 items-end h-32">
              <div v-for="point in tmResult.derating_curve" :key="point.temp"
                class="flex-1 flex flex-col items-center">
                <div class="w-full rounded-t"
                  :style="{ height: point.power_pct + '%', backgroundColor: point.power_pct > 80 ? '#10B981' : point.power_pct > 50 ? '#F59E0B' : '#EF4444' }">
                </div>
    ...
    <div v-show="activeModule === 'scada'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">SCADA/EMS设计</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label class="block text-xs mb-1" style="color: #999;">集装箱数量</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">通信协议</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">调度策略</label>
        ...
        <button @click="designScada" :disabled="loading"
          class="px-6 py-2 rounded-lg text-white font-medium text-sm" style="background-color: #2F5496;">
        ...
        <div v-if="seResult" class="mt-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value">{{ seResult.total_data_points }}</div><div class="result-label">总数据点</div></div>
            <div class="result-card"><div class="result-value">{{ seResult.analog_points }}</div><div class="result-label">模拟量</div></div>
            <div class="result-card"><div class="result-value">{{ seResult.digital_points }}</div><div class="result-label">数字量</div></div>
            <div class="result-card"><div class="result-value">{{ seResult.control_points }}</div><div class="result-label">控制点</div></div>
          </div>
          <div class="p-4 bg-gray-50 rounded-lg">
            <div class="text-sm font-bold mb-2" style="color: #666;">系统架构</div>
    ...
    <div v-show="activeModule === 'hv'" class="space-y-4">
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        <h3 class="text-lg font-bold mb-4" style="color: #2F5496;">高压接入设计</h3>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            <label class="block text-xs mb-1" style="color: #999;">总功率 (MW)</label>
          ...
          <div>
            <label class="block text-xs mb-1" style="color: #999;">短路容量 (MVA)</label>
            <input v-model.number="hvForm.short_circuit_capacity_mva" type="number" class="form-input" placeholder="500" />
          </div>
          <div>
            <label class="block text-xs mb-1" style="color: #999;">并网点类型</label>
        ...
        <button @click="designHV" :disabled="loading"
          class="px-6 py-2 rounded-lg text-white font-medium text-sm" style="background-color: #2F5496;">
        ...
        <div v-if="hvResult" class="mt-6 space-y-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div class="result-card"><div class="result-value">{{ hvResult.transformer_count }}</div><div class="result-label">变压器数量</div></div>
            <div class="result-card"><div class="result-value">{{ hvResult.transformer_capacity_mva }}MVA</div><div class="result-label">变压器容量</div></div>
            <div class="result-card"><div class="result-value">{{ hvResult.transformer_ratio }}</div><div class="result-label">变比</div></div>
            <div class="result-card"><div class="result-value">{{ hvResult.mv_breaker_rating_ka }}kA</div><div class="result-label">断路器额定值</div></div>
          </div>
          <div v-if="hvResult.protection_scheme" class="p-4 bg-gray-50 rounded-lg">
            <div class="text-sm font-bold mb-2" style="color: #666;">保护配置</div>
            <div class="grid grid-cols-2 md:grid-cols-5 gap-2">
              <div v-for="prot in hvResult.protection_scheme" :key="prot.name"
                class="text-xs bg-white rounded p-2 border" style="border-color: #E0E0E0;">
      ...
      <div class="bg-white rounded-lg p-6 shadow-sm border" style="border-color: #E0E0E0;">
        ...
        <div v-if="bidResult" class="mt-4">
          <div v-for="chapter in bidResult.chapters" :key="chapter.num" class="mb-4">
            <div class="font-bold text-sm p-2 rounded" style="background-color: #E8EEF5; color: #2F5496;">
              {{ chapter.num }}. {{ chapter.title }}
            </div>
            <div v-for="sec in chapter.sections" :key="sec.num" class="ml-4 mt-2 p-3 bg-gray-50 rounded">
              <div class="font-medium text-sm" style="color: #333;">{{ sec.num }} {{ sec.title }}</div>
        ...
        <!-- 交互式图表预览 -->
        <div class="mt-6 pt-4 border-t" style="border-color: #E0E0E0;">
          <div class="flex items-center justify-between mb-3">
            <h4 class="text-base font-bold" style="color: #2F5496;">交互式图表预览</h4>
            <span class="text-xs" style="color: #999;">基于 Plotly，支持缩放/悬停/导出</span>
          </div>
          <div class="flex flex-wrap gap-2 mb-3">
            <button v-for="c in chartTypes" :key="c.id"
              ...
              :style="activeChart === c.id
                ? { backgroundColor: '#2F5496', color: '#fff', borderColor: '#2F5496' }
                : { backgroundColor: '#fff', color: '#2F5496', borderColor: '#2F5496' }">
              {{ chartLoading === c.id ? '加载中...' : c.label }}
            </button>
          </div>
          <div v-if="chartError" class="text-xs p-2 rounded mb-3" style="background-color: #FEF2F2; color: #DC2626;">
            {{ chartError }}
          </div>
          <div v-if="chartHtml" ref="chartContainer" class="border rounded-lg p-2" 
            style="border-color: #E0E0E0; background-color: #fff;" v-html="chartHtml">
          </div>
          <div v-else-if="!chartLoading" class="text-xs text-center py-8" style="color: #999;">
