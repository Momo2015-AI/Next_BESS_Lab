<template>
  <div class="rounded-lg p-3" style="background-color: var(--color-card); border: 1px solid var(--color-border);">
    <h3 class="font-bold text-xs mb-2 flex items-center gap-2" style="color: var(--color-text);">
      <span class="w-5 h-5 rounded text-[10px] flex items-center justify-center font-bold" style="background-color: rgba(245, 158, 11, 0.2); color: var(--color-warning);">VII</span>
    ...
    <div class="space-y-3 text-[10px]" style="color: var(--color-text-secondary);">
      ...
      <div class="grid grid-cols-3 gap-2">
        <div>
          ...
          <select v-model="selectedCell" class="w-full rounded px-2 py-1 text-xs" style="background-color: var(--color-input-bg-dark); border: 1px solid var(--color-input-border); color: var(--color-text);" @change="updateCAPEX">
        ...
        <div>
          ...
          <select v-model="selectedContainer" class="w-full rounded px-2 py-1 text-xs" style="background-color: var(--color-input-bg-dark); border: 1px solid var(--color-input-border); color: var(--color-text);" @change="updateCAPEX">
            <option value="">选择集装箱</option>
            <option v-for="container in availableContainers" :key="container.id" :value="container.id">
              {{ container.mfr }} {{ container.model }} ({{ container.ratedEnergyMWh }}MWh, ¥{{ container.unitPrice }}/MWh)
            </option>
          </select>
        </div>
        <div>
          <label class="block mb-0.5" style="color: var(--color-text-muted);">PCS 变流器</label>
          <select v-model="selectedPcs" class="w-full rounded px-2 py-1 text-xs" style="background-color: var(--color-input-bg-dark); border: 1px solid var(--color-input-border); color: var(--color-text);" @change="updateCAPEX">
      ...
      <!-- 配置摘要 -->
      <div v-if="hasSelection" class="border rounded p-2" style="border-color: var(--color-border); background-color: var(--color-card-dark);">
        <h4 class="text-xs font-bold mb-2" style="color: var(--color-text-secondary);">当前配置 Current Configuration</h4>
        <div class="grid grid-cols-3 gap-2 text-xs">
          <div>
            <div style="color: var(--color-text-muted);">电芯</div>
            <div class="font-mono mt-0.5" style="color: var(--color-text);">{{ selectedCellInfo?.mfr }} {{ selectedCellInfo?.model }}</div>
            <div class="text-[9px]" style="color: var(--color-text-muted);">{{ selectedCellInfo?.capacityAh }}Ah @ ¥{{ selectedCellInfo?.unitPrice }}/Ah</div>
          </div>
          <div>
            <div style="color: var(--color-text-muted);">集装箱</div>
            <div class="font-mono mt-0.5" style="color: var(--color-text);">{{ selectedContainerInfo?.mfr }} {{ selectedContainerInfo?.model }}</div>
            <div class="text-[9px]" style="color: var(--color-text-muted);">{{ selectedContainerInfo?.ratedEnergyMWh }}MWh @ ¥{{ selectedContainerInfo?.unitPrice }}/MWh</div>
          </div>
          <div>
            <div style="color: var(--color-text-muted);">PCS</div>
            <div class="font-mono mt-0.5" style="color: var(--color-text);">{{ selectedPcsInfo?.mfr }} {{ selectedPcsInfo?.model }}</div>
            <div class="text-[9px]" style="color: var(--color-text-muted);">{{ selectedPcsInfo?.ratedPowerMW }}MW @ ¥{{ selectedPcsInfo?.unitPrice }}/MW</div>
          </div>
        </div>
      </div>
      ...
      <div v-if="hasSelection" class="border-t pt-2">
        <h4 class="text-xs font-bold mb-2" style="color: var(--color-text-secondary);">CAPEX 成本计算 CAPEX Cost Calculation</h4>
        <div class="space-y-1 text-xs">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-mono" style="color: var(--color-text-secondary);">{{ formatCurrency(containerCost) }}</span>
          </div>
          <div class="flex justify-between">
...
const selectedCellInfo = computed(() => 
  availableCells.value.find(c => c.id === selectedCell.value)
)

const selectedContainerInfo = computed(() => 
  availableContainers.value.find(c => c.id === selectedContainer.value)
)

const selectedPcsInfo = computed(() => 
  availablePcs.value.find(p => p.id === selectedPcs.value)
)

const hasSelection = computed(() => 
  selectedCell.value && selectedContainer.value && selectedPcs.value
)
...
const cellCost = computed(() => {
  ...
  return selectedCellInfo.value.unitPrice * selectedCellInfo.value.capacityAh * cellsPerContainer * selectedContainerInfo.value.ratedEnergyMWh
})
